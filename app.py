...
if "results" in st.session_state:
    ...
    if not results:
        ...
        # In the display section, add event_type column
        df = pd.DataFrame([r.model_dump() for r in results])
        df["Event Type"] = df["event_type"].map(EVENT_COLORS) + " " + df["event_type"].str.replace("_", " ").str.title()
        ...
        # Reorder
        df = df[["Verbatim", "Event Type", "Decode", "LLT Code", "PT Code", "Search Tier", "Confidence"]]
